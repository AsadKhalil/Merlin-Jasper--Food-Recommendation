...
#%%
branch_df = pd.read_csv(gap('branches.csv'), usecols=['id', 'restaurant_id', 'name'])
...
#%%
usecols=['id', 'quantity', 'category_id', 'branch_id', 'status', 'schedule_on', 'company_id', 'building_id', 'created_at']
orders_df = pd.read_csv(gap('orders.csv'), usecols=usecols, parse_dates=['schedule_on'])
orders_df['created_at'] = pd.to_datetime(orders_df['created_at']).dt.normalize()
orders_df.rename(columns={'id': 'order_id'}, inplace=True)
orders_df = orders_df.loc[orders_df['schedule_on'] >= START_DATE]
orders_df = orders_df.loc[orders_df['created_at'] <= training_max_date]
if orders_df[orders_df['created_at'] == training_max_date].shape[0] == 0:
    print('maximum training data is not available in orders')
    assert False, 'maximum training data is not available in orders'
...
temp_df = companies_schedule_df.copy().append(building_schedule_df)[['schedule_on', 'restaurant_id', 'client_id']]
...
temp_df = temp_df.loc[temp_df['schedule_on'] <= training_max_date]
