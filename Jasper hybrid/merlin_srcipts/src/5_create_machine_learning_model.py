...
if args.force_start_date is not None:
    ...
    weekday = training_max_date.weekday() + 1
    week_day_diff = weekday % 7
    test_start_date = training_max_date + relativedelta(days = (week_day_diff * -1))
    test_end_date = training_max_date
    train_end_date = test_start_date + relativedelta(days=-1)
    # start_date = test_start_date.strftime('%Y-%m-%d')
    # test_end_date = prediction_start_date + relativedelta(days=-1)
    print(f'Test data duration {test_start_date} to {test_end_date}')
    print(f'Train data end date {train_end_date}')
...
df = df[df['category'].isin([1, 8, 9, 11] + list(range(14, 26)))]
df = df[df['type'].isin(list(range(4, 10)))]
...
drop = [ 'rating_amax_restaurant', 'exp_orders_median_restaurant', 'orders_medianFalse', 'exp_orders_amaxFalse', 'orders_kurtosis', 'exp_orders_skewFalse',
 ...
 'orders_mean',
 'orders_median',
 'exp_orders_median', 'area_id', 'c2r_expected_vs_deliver_diff_shift1',]
...
client_areas_df = companies_df.append(buildings_df)
...
results.to_csv(gapp('clientId_sample_predictions.csv'), index=False)
