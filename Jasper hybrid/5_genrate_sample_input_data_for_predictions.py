...
dropped_clients_combined = dropped_comps_df.copy() # "remove code:" pd.concat([dropped_comps_df, dropped_builds_df], ignore_index=True, sort=False)
#%%
...
restaurants_df_1 = pd.read_csv(gap('restaurants.csv'), usecols=['id', 'status', 'name'])
restaurants_df_1.rename(columns={'id': 'restaurant_id'}, inplace=True)
restaurant_details_df = pd.read_csv(gap('restaurant_details.csv'), usecols=['restaurant_id', 'merlin_3_type', 'merlin_3_category']).rename(columns={'merlin_3_category': 'category', 'merlin_3_type': 'type'})
restaurants_df_1 = pd.merge(restaurants_df_1, restaurant_details_df, on='restaurant_id',how='left')


restaurants_df = pd.read_csv(gap('restaurants.csv'), usecols=['id', 'status', 'name'])
restaurants_df.rename(columns={'id': 'restaurant_id'}, inplace=True)
restaurant_details_df = pd.read_csv(gap('restaurant_details.csv'), usecols=['restaurant_id', 'merlin_3_type', 'merlin_3_category']).rename(columns={'merlin_3_category': 'category', 'merlin_3_type': 'type'})
restaurants_df = pd.merge(restaurants_df, restaurant_details_df, on='restaurant_id',how='left')


#%%
# Dropping nan restaurants
dropped_nan_restos = restaurants_df[restaurants_df.isnull().any(axis=1)]
# Dropping invalid type restaurants
dropped_invalid_type_restos = restaurants_df[~restaurants_df['type'].isin([1,5,6,7,9])].dropna()
# Dropping invalid category restaurants
dropped_invalid_category_restos = restaurants_df[~restaurants_df['category'].isin([1,2,8,14,17,18,21,22,23,26,27,28])].dropna()
...
restaurants_df = restaurants_df[restaurants_df['type'].isin([1,5,6,7,9])]
restaurants_df = restaurants_df[restaurants_df['category'].isin([1,2,8,14,17,18,21,22,23,26,27,28])]
...
# restaurants_details_df = pd.read_csv(gap('restaurants.csv'), usecols=['id', 'type', 'category'])
# restaurants_details_df.rename(columns={'id': 'restaurant_id'}, inplace=True)
# dropped_branches_wo_valid_restos = dropped_branches_wo_valid_restos.merge(restaurants_details_df, on='restaurant_id', how='left')

dropped_branches_wo_valid_restos = dropped_branches_wo_valid_restos.rename(columns={'name': 'restaurant_name', 'status': 'resto_status', 'type': 'resto_type', 'category': 'resto_category'})
...
dropped_branches_wo_valid_resto = dropped_branches_wo_valid_resto.rename(columns={'status': 'resto_status', 'name': 'restaurant_name', 'type': 'resto_type', 'category': 'resto_category'})
...
prepared_data.rename(columns={'restaurant_category': 'category', 'restaurant_type':'type'}, inplace=True)
