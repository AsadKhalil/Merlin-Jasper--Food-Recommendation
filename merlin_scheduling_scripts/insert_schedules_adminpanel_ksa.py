...
TARGET_OUTPUT_DIR = '/home/KSA/ds_active_work_space/data/processed/v6/output'
# TARGET_OUTPUT_DIR = '/home/munchon/Documents/Munir/Documents/merlin_py/data/processed/v6/output'
...
comps_df = pd.read_csv(gap('companies.csv'), usecols=['id' , 'drop_off_point_id', 'name', 'area_id', 'status', 'is_active_virtual', 'group_id'])
menus_df = pd.read_csv(gap('menus.csv'), usecols=['id', 'name', 'restaurant_id', 'category_id', 'status', 'merlin_status'])
pred_df = pd.read_csv(gapp('ksa_ots.csv'), usecols=['branch_id', 'schedule_on', 'restaurant_id', 'ots'])
...
def get_menu_schedules_df(input_file, comps_df):
    ...
    comps_df["name"] = comps_df["name"].str.lower()
    comps_df = comps_df[~comps_df["name"].str.contains('test')]

    comps_merged = schedule_pairs_df.merge(comps_df, on="group_id", how="left").rename(columns={'id': 'company_id'})
    comps_merged = comps_merged[['schedule_on', 'branch_id', 'company_id', 'group_id', 'restaurant_id']]
    ...
    order_merged_df = df_merged.merge(pred_df.drop_duplicates(subset=["branch_id", "restaurant_id", "schedule_on"]), on=["branch_id", "restaurant_id", "schedule_on"], how="left")
...
def mysql_dump_companies(comps_df):
    ...
    with sql_engine.begin() as e_con:
        sql = """DROP TABLE IF EXISTS temp_menu_schedules"""
        e_con.execute(sql)
...
if __name__ == "__main__":
    ...
    if args.insert_company_schedules_adminpanel:
       comps_df = get_menu_schedules_df(INPUT_FILE, comps_df)
